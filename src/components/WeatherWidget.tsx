...
const WeatherWidget = () => {
  ...
  return <div className={isDarkMode ? "dark" : ""}>
      <div className="min-h-screen flex items-center justify-center bg-background p-4 transition-colors duration-500">
        {/* Dark mode toggle */}
        <button onClick={() => setIsDarkMode(!isDarkMode)} className="fixed top-6 right-6 z-50 px-4 py-2 rounded-pill text-sm font-medium
        ...
        <div className="relative w-[420px] rounded-widget overflow-hidden shadow-widget transition-all duration-500">
          ...
          {/* Rain / Storm effects */}
          {isRainy && <RainEffect intensity={data.weather === "stormy" ? "heavy" : "moderate"} withLightning={data.weather === "stormy"} />}
          ...
          <div className="relative z-10 p-6 pb-4">
            ...
            <div className="relative mt-2">
              {/* Sun sphere */}
              <div className="absolute top-2 left-0 w-12 h-12 rounded-full animate-sun-pulse z-10 transition-opacity duration-700" style={{
              ...
              <div className="relative px-2">
                ...
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-center">
                  <p className="text-primary-foreground/70 text-xs font-semibold">11:21 AM</p>
                  <p className="text-primary-foreground/50 text-[10px]">Fev 2, 2025</p>
                </div>
              </div>

              {/* Temperature */}
              <div className="mt-1 bg-glass-strong backdrop-blur-[16px] rounded-card p-4 border border-glass-border shadow-glass-card transition-all duration-500">
                <div className="text-secondary-foreground transition-all duration-500">
                  <span className="font-light leading-none text-5xl">{data.temp}</span>
                  <span className="text-2xl font-light align-top">°</span>
                </div>
                <p className="text-secondary-foreground/90 text-sm font-medium mt-1">
                  {data.desc}
                </p>
              </div>
            </div>
              ...
              <div className="bg-glass-strong backdrop-blur-[16px] rounded-card p-4 border border-glass-border shadow-glass-card transition-all duration-500">
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                    <div className="h-full bg-primary rounded-full transition-all duration-700" style={{
                    width: `${data.cloud}%`
                  }} />
                  </div>
                </div>
            ...
            <div className="mt-5 flex items-center justify-around bg-nav backdrop-blur-[24px] rounded-pill py-2.5 px-3 border border-glass-border shadow-glass-card">
              {WEEKDAYS.map((day, i) => <button key={day} onClick={() => setSelectedDay(i)} className={`relative px-3 py-1.5 rounded-pill text-xs font-semibold transition-all duration-300
                    ${selectedDay === i ? "bg-primary text-primary-foreground shadow-sm scale-105" : "text-nav-icon hover:bg-glass hover:scale-105"}
